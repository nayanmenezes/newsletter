...
const NewsletterForm = () => {
  ...
  const handleSubmit = async (e: FormEvent<HTMLFormElement>) => {
    ...
    const mailchimpCall = await fetch("/api/addSubscription", {
      body: JSON.stringify({ email }),
      headers: { "Content-Type": "application/json" },
      method: "POST",
    });

    const dynamoCall = await fetch("/api/insertEmail", {
      body: JSON.stringify({ email }),
      headers: { "Content-Type": "application/json" },
      method: "POST",
    });

    const mailchimpData = await mailchimpCall.json();
    const dynamoData = await dynamoCall.json();

    if (mailchimpData.error) {
    ...
    if (dynamoData.error) {
      console.log(dynamoData.error);
      return;
    }

    setSuccessMessage(mailchimpData as MembersSuccessResponse);
            ...
            <div className="text-xs sm:text-sm text-[#4B4C52]">
              {successMessage ? (
                <p>
                  ...
                  to our newsletter. We&apos;ll let you know when we launch!
                </p>
              ) : (
                <p>
                  You are already added to our newsletter We&apos;ll let you
                  know when we launch!
                </p>
              )}
            </div>
